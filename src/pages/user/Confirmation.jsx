...
const Confirmation = () => {
      ...
      try {
        ...
        const confirmationRes = await fetch('http://localhost:5000/api/confirmations/create', {
          ...
          body: JSON.stringify({
            ...
            expiryDate: new Date(), // You can use expiryDate below too
            customerId: customerId
          })
        });

        const savedConfirmation = await confirmationRes.json();
        console.log('✅ Confirmation Saved:', savedConfirmation);
      } catch (error) {
        console.error('❌ Error saving customer and confirmation:', error);
      }
    };
  ...
  // Display expiry date: 3 years from today
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-3xl">
        <div className="bg-white rounded-lg shadow-md p-6 text-center">
          ...
          <div className="flex flex-col md:flex-row justify-center space-y-4 md:space-y-0 md:space-x-4">
            <button className="flex items-center justify-center px-6 py-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">
